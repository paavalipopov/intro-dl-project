...
class Experiment(IExperiment):
    ...
    def get_resumed_params(self, path):
        ...
        if config["mode"] == "tune":
            try:
                self.start_k = len(glob.glob(f"{path}/k_*")) - 1
                last_fold_dir = sorted(glob.glob(f"{path}/k_*"))[-1]
                try:
                    with open(last_fold_dir + "/runs.csv", "r") as fp:
                        self.start_trial = len(fp.readlines()) - 1
                except FileNotFoundError:
                    self.start_trial = 0
                faildir = last_fold_dir + f"/trial_{self.start_trial:04d}"
                ...
                self.start_k, self.start_trial = 0, 0
    ...
    def on_experiment_start(self, exp: "IExperiment"):
        ...
        if self.mode == "tune":
            wandb_run_name = (
                f"k_{self.k:02d}-trial_{self.trial:04d}-in_k_{self.in_k:02d}"
            )
            self.runpath = f"{self.logdir}/k_{self.k:02d}/trial_{self.trial:04d}/in_k_{self.in_k:02d}"

            self.cv_results_path = (
                f"{self.logdir}/k_{self.k:02d}/trial_{self.trial:04d}/runs.csv"
            )
    ...
    def initialize_dataset(self):
        ...
        if self.mode == "tune":
            ...
            train_index, test_index = inner_CV_folds[self.in_k]
        ...
        # train/val split
        n_observations = (
            self.data_shape[0] if self.mode == "experiment" else X_train.shape[0]
        )
        X_train, X_val, y_train, y_val = train_test_split(
            X_train,
            y_train,
            test_size=n_observations // self.n_splits,
    ...
    def start(self):
        ...
        if self.mode == "tune":
            ...
            for k in range(self.start_k, self.n_splits):
                self.k = k
                if k == self.start_k:
                ...
                for trial in range(start_trial, self.n_trials):
                    self.trial = trial
                    self.model_config = get_tune_config(
                        self, random_seed=int(time.time())
                    )
                    for in_k in range(self.n_splits):
                        self.in_k = in_k
                        self.run()
                    ...
                    with open(f"{self.logdir}/k_{self.k:02d}/runs.csv", "a") as f:
                        df.to_csv(f, header=f.tell() == 0, index=False)
            ...
            # (determined by 'trial')
            self.k = self.start_k
            self.model_config = get_best_config(self, self.k)
